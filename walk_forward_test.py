...
def run_simulation():
    ...
    gp, gl = sum(t['profit'] for t in wins), abs(sum(t['profit'] for t in losses))
    pf = gp / gl if gl > 0 else float('inf')
    bench_ret = (benchmark_df['Close'].iloc[-1] / benchmark_df['Close'].iloc[0]) - 1

    print(f"【総合パフォーマンス】\n ├ 初期資金    : {INITIAL_CAPITAL:,.0f} 円\n ├ 最終総資産  : {final_eq:,.0f} 円")
    print(f" ├ 純利益      : {final_eq - INITIAL_CAPITAL:>+,.0f} 円 ({total_ret:>+5.1%})\n ├ 最大DD(MDD) : {mdd:>+5.1%}\n └ TOPIX比較   : {bench_ret:>+5.1%}")
    print(f"\n【トレード詳細】\n ├ 総取引回数  : {len(trade_history)} 回\n ├ 勝率        : {win_rate:>5.1%} ({len(wins)}勝 / {len(losses)}敗)")
    print(f" ├ 平均利益    : {gp/len(wins) if wins else 0:>+,.0f} 円\n ├ 平均損失    : {-gl/len(losses) if losses else 0:>+,.0f} 円\n └ プロフィット・ファクター (PF) : {pf:.2f}")
